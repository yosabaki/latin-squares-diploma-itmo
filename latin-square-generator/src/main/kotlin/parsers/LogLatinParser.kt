...
class LogLatinParser(val q: Int, val matrixCount: Int, reader: BufferedReader, format: OutputFormat) :
    Parser(reader, format) {

    override fun construct(variables: List<Boolean>) : List<List<List<Int>>> {
        val matrixes = (0 until matrixCount).map { initMatrix(q, q, log2(q)) }.map { matrix ->
            matrix.map { line ->
                line.map { vars ->
                    parseIntLog(readNVars(log2(q)), vars) + 1
                }
            }
        }
