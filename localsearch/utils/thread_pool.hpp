...
public:
    ...
    void wait_for_tasks_conditional() {
        while (true) {
            if (tasks_total == 0) {
                break;
            }
            notifier.wait(conditional_lock);
        }
    }
...
private:
    ...
    std::mutex conditional_mutex = std::mutex();

    std::unique_lock<std::mutex> conditional_lock = std::unique_lock(conditional_mutex);
